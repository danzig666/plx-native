...
- **No auto-skip intro / credits preference** — `minor` / `small`  
  *Status (fork):* INTRO half done — Settings → Playback → "Skip intros automatically" takes an intro offer at its edge (`app/run.rs`, `screens/player/skip_pill.rs::auto_skip`) and shows an "Intro skipped" notice. Credits remain manual.  
...
- **No auto-skip intro/credits setting** — `minor` / `small`  
  *Status (fork):* intro done (see the entry above); credits remain manual.  
...
- **No auto-skip preference for intro/credits** — `minor` / `medium`
  *Status (fork):* intro done (see the first auto-skip entry); credits remain manual.  
